...
- `test_smoke.py` — 21 tests: parametrized router import (19 routers — `vip` added in scenario M) + `api.main.app` instantiation + `get_settings()`.
...
3. **Import smoke**: usually automatic — `tests/test_smoke.py` already parametrizes over the 19 router names. If a new router is added, append it to the list.
