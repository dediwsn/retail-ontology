...
Neptune trend aggregation → Sonnet 4.6 **token-streamed Korean summary** → **AgentCore Code
Interpreter** (Firecracker microVM) executing real matplotlib code with bundled NanumGothic fonts to
return a genuine PNG chart — not an LLM-hallucinated description of a chart. Plus a 1-hop drilldown
subgraph.
...
| Charts | AgentCore Code Interpreter | Executes real matplotlib in a Firecracker microVM — the chart is computed, not described. |
...
| FR-04 | The system SHALL render analytical charts by executing code in an isolated sandbox, not by describing them. | C |
