...
- **MD Insights (C)** — Neptune trend aggregation, Sonnet 4.6 streaming Korean summary, AgentCore Code Interpreter rendering real matplotlib PNG charts with NanumGothic Korean glyphs, plus 1-hop drilldown subgraph.
...
- **MD 인사이트 (C)** — Neptune 트렌드 집계 + Sonnet 4.6 토큰 스트리밍 한국어 요약 + AgentCore Code Interpreter가 NanumGothic 한글 폰트로 실제 matplotlib PNG를 렌더링하고 1-hop 드릴다운 부분그래프를 함께 제공합니다.
